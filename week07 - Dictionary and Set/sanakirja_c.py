...
Solution of task 7.7 -- More Features for the Tourist Dictionary 2:
...
def reverse_dict(dictionary):
    """Reverses the dictionary so that the keys become values and the values
    become keys.

    @param dictionary: The dictionary to be reversed.
...
def main():
    ...
    while True:
        ...
        if command == "W":
            ...
            # Print the words (english spanish) in alphabetical order.
            print("\nEnglish-Spanish")
            for word in sorted(english_spanish):
                print(word, english_spanish[word])
            # Then print the words (spanish english) in alphabetical order.
            print("\nSpanish-English")
            spanish_english=reverse_dict(english_spanish)
